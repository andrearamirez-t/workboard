...
export default function Login() {
  const { user, loading, loginWithGoogle, authError, isSuperAdmin, role } = useAuth()
  const navigate = useNavigate()

  useEffect(() => {
    if (!loading && user && role) {
      const dest = isSuperAdmin ? "/overview" : "/semilleros"
      navigate(dest, { replace: true })
    }
  }, [user, loading, role, isSuperAdmin, navigate])
          ...
          <div className="relative bg-card border border-border rounded-3xl p-8 space-y-7"
            ...
            <div className="text-center space-y-4">
              ...
              <div>
                ...
                <p className="text-[13px] text-muted-foreground mt-0.5">Gestión de equipos</p>
              </div>
            </div>
